    ...
    <div className={`bg-black border-2 border-[#fcb131] rounded-lg p-6 space-y-6 ${className}`}>
      {showTitle && (
        <div className="text-center">
          <h3
            className="text-lg font-bold text-[#fcb131] mb-2"
            style={{ fontFamily: "'Press Start 2P', monospace" }}
          >
            Wallet Connection Issues?
          </h3>
          <p className="text-sm text-[#fcb131] opacity-80">
      ...
      {environmentIssues.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-[#fcb131]">Detected Issues:</h4>
          {environmentIssues.map((issue, index) => (
            ...
            >
              <h5 className="text-sm font-semibold text-[#fcb131] mb-1">{issue.title}</h5>
              <p className="text-xs text-gray-300 mb-2">{issue.description}</p>
              <p className="text-xs text-[#fcb131] opacity-80">{issue.solution}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Connection Options */}
      <div className="space-y-3">
        <h4 className="text-sm font-semibold text-[#fcb131]">Try These Solutions:</h4>

        {/* Quick Fix Button */}
        <button
          onClick={() => attemptConnection('auto')}
          disabled={isAttempting !== null}
          className="w-full px-4 py-3 bg-gradient-to-r from-[#fcb131] to-[#f39c12] text-black font-bold rounded-lg hover:from-[#f39c12] hover:to-[#fcb131] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center space-x-2"
          style={{
        ...
        {isFarcaster && (
          <div className="p-3 bg-purple-900/20 border border-purple-700 rounded">
            <h5 className="text-sm font-semibold text-[#fcb131] mb-2">🎯 Farcaster Users:</h5>
        ...
        {isBrave && (
          <div className="p-3 bg-orange-900/20 border border-orange-700 rounded">
            <h5 className="text-sm font-semibold text-[#fcb131] mb-2">🦁 Brave Browser:</h5>
            <p className="text-xs text-gray-300 mb-2">
        ...
        <button
          ...
          className="w-full text-xs text-[#fcb131] opacity-60 hover:opacity-80 transition-opacity"
        >
