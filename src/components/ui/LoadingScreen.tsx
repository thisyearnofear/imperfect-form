...
const LoadingScreen: React.FC<LoadingScreenProps> = ({
  ...
  }, [currentPhase, autoHide, hideDelay, onComplete, poseState, loadingInstructions]);
  ...
  return (
    <div className="loading-container flex flex-col items-center justify-center h-full">
      <div className="loading-text text-3xl font-bold mb-6 text-fcb131">Imperfect Form</div>

      {/* Progress indicator */}
      <ProgressIndicator phase={currentPhase} className="mb-6" />

      {/* Phase indicator */}
      <div className={`mb-4 p-3 rounded-lg ${phaseInfo.bgColor} border border-opacity-30`}>
        <div className={`text-2xl ${phaseInfo.color} text-center`}>{phaseInfo.icon}</div>
      </div>
      ...
      <MotivationalMessages phase={currentPhase} className="mt-6 text-yellow-200" />
