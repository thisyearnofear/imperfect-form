...
export function NotificationSignup({
    ...
    switch (trigger) {
      case 'workout_complete':
        return {
          ...
          description: 'Get notified about new challenges, achievements, and daily motivation',
        ...
        <div className="bg-gradient-to-r from-purple-900 to-pink-900 border border-purple-500/50 rounded-lg p-4 shadow-lg">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              <div className="font-bold text-white text-sm mb-1">{content.title}</div>
              <div className="text-xs text-purple-200">{content.description}</div>
            </div>
            <button onClick={handleDismiss} className="text-gray-400 hover:text-white ml-2">
              ✕
            </button>
          </div>
  ...
  if (variant === 'inline') {
    return (
      ...
      >
        <div className="flex-1">
          <div className="text-sm font-medium text-white">{content.subtitle}</div>
          <div className="text-xs text-purple-200">{content.description}</div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        <AddMiniAppButton variant="primary" showAfterWorkout={true} className="w-full" />
