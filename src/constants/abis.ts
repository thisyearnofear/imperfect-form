/**
 * Contract ABIs — pure data.
 *
 * Kept separate from contracts.ts so the ethers-using recap/leaderboard
 * modules (which import ABIs via contracts.ts's re-export) never pull the
 * ethers library into the / first load — the ABIs have no eager importers of
 * their own, so ethers stays in the lazy chunks (PERFORMANT / PREVENT BLOAT).
 */
