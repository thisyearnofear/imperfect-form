...
export const EnhancedChainThemeProvider: React.FC<EnhancedChainThemeProviderProps> = ({ 
  ...
  const setTheme = useCallback((id: ChainId) => {
  ...
  const getSpacing = useCallback((size: keyof ChainTheme['spacing']) => {
    return getThemeSpacing(currentTheme, size);
  }, [currentTheme]);

  const getRadius = useCallback((size: keyof ChainTheme['borderRadius']) => {
    return getThemeRadius(currentTheme, size);
  }, [currentTheme]);

  const getShadow = useCallback((type: keyof ChainTheme['shadows']) => {
    return getThemeShadow(currentTheme, type);
  }, [currentTheme]);

  const getButtonThemeConfig = useCallback((variant: ComponentVariant, size: ComponentSize) => {
