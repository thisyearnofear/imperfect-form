...
export function WalletProviderProvider({ children }: WalletProviderProps) {
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          if (thirdwebObj?.auth?.logout) {
            ...
            console.log("WalletProviderContext: Disconnecting from ThirdWeb (alt method)");
            thirdwebObj.logout();
          }
          ...
          console.error("WalletProviderContext: Error during provider disconnect:", error);
        }
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === "selectedWalletProvider") {
        ...
        if (newProvider !== walletProvider) {
          console.log(`WalletProviderContext: Provider changed to ${newProvider || 'null'} from storage event`);
  ...
  useEffect(() => {
    ...
    if (walletProvider === null) {
      ...
      if (typeof window !== "undefined") {
        console.log("WalletProviderContext: Wallet provider set to null, clearing connection state");
      }
    } else {
      // Log the active wallet provider for debugging
      if (typeof window !== "undefined") {
        console.log(`WalletProviderContext: Wallet provider set to ${walletProvider}`);
