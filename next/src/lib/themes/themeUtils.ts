...
export const generateCSSCustomProperties = (theme: ChainTheme): CSSCustomProperties => {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(`Generated ${Object.keys(properties).length} CSS custom properties for theme: ${theme.id}`);
...
export const validateTheme = (theme: DeepPartial<ChainTheme>): ThemeValidationResult => {
  ...
  if (theme.palette) {
    ...
    if (theme.palette.text && theme.palette.background) {
      ...
      if (contrast < 4.5) {
        warnings.push(`Text/background contrast ratio (${contrast.toFixed(2)}) is below WCAG AA standard (4.5)`);
      }
    }
  }
  ...
  if (theme.components) {
    ...
    if (theme.components?.button) {
      if (!theme.components.button.variants) {
        ...
        const requiredVariants: ComponentVariant[] = ['primary', 'secondary', 'accent', 'ghost', 'outline'];
...
const isValidAnimationTiming = (timing: string): boolean => {
  ...
  return validTimings.includes(timing) || /^cubic-bezier\([\d.,\s-]+\)$/.test(timing) || /^steps\(\d+/.test(timing);
};
...
export const createThemeFallback = (partialTheme: DeepPartial<ChainTheme>): ChainTheme => {
  ...
  requiredPaletteKeys.forEach(key => {
    if (!fallbackTheme.palette[key as keyof ColorPalette]) {
      fallbackTheme.palette[key as keyof ColorPalette] = DEFAULT_THEME.palette[key as keyof ColorPalette];
    }
  });
...
export const mergeThemes = (baseTheme: ChainTheme, overrides: DeepPartial<ChainTheme>): ChainTheme => {
...
export const getThemeRadius = (theme: ChainTheme, size: keyof ChainTheme['borderRadius']): string => {
