...
export class EngagementTracker {
    ...
    switch (event.eventType) {
      ...
      case 'workout_completed':
        ...
        if (event.metadata.duration && typeof event.metadata.duration === 'number') {
          ...
          stats.averageWorkoutDuration = (totalDuration + event.metadata.duration) / stats.totalWorkouts;
        }
        break;
  ...
  static async getAnalytics(): Promise<EngagementAnalytics> {
    ...
    // Calculate retention
    const day1Retention = allStats.filter(s => s.retentionDays >= 1).length / Math.max(allStats.length, 1);
    const day7Retention = allStats.filter(s => s.retentionDays >= 7).length / Math.max(allStats.length, 1);
    const day30Retention = allStats.filter(s => s.retentionDays >= 30).length / Math.max(allStats.length, 1);
