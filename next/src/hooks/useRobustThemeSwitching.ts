...
export function useRobustThemeSwitching(): UseRobustThemeSwitchingReturn {
  ...
  const getAvailableThemes = useCallback(() => {
    return [
      ...
      { id: 'polygon' as ChainId, name: 'Polygon', chainId: chainConfigs[SupportedChain.POLYGON].id },
  ...
  const applyThemeWithRetry = useCallback(async (
    themeId: ChainId, 
    attemptCount = 0
  ): Promise<boolean> => {
    try {
  ...
  const switchToTheme = useCallback(async (themeId: ChainId, options: { silent?: boolean } = {}): Promise<boolean> => {
    ...
    try {
      ...
      if (shouldShowNotifications) {
        toast.error(`Error switching theme: ${error instanceof Error ? error.message : 'Unknown error'}`, { id: toastId });
      }
      return false;
    }
  ...
  const switchToChain = useCallback(async (chainId: number, options: { silent?: boolean } = {}): Promise<boolean> => {
  ...
  }, [wallet.chainId, wallet.isConnected, currentTheme.id, getThemeForChain, switchToTheme, state.isLoading]);
