    ...
    <div className={`fixed bottom-4 left-4 z-50 ${className}`}>
      <div className={`p-3 rounded-lg border text-xs font-mono ${getStatusColor()}`}>
        <div className="space-y-1">
          ...
          {wallet.address && (
            <div>
              <span className="font-bold">Address:</span> {wallet.address.slice(0, 6)}...{wallet.address.slice(-4)}
            </div>
          )}
          ...
          {user && (
            <div>
              <span className="font-bold">User:</span> {user.displayName || user.username || "Unknown"}
            </div>
          )}
