...
import { POLYGON_CONTRACT_ADDRESS, BASE_CONTRACT_ADDRESS, fitnessLeaderboardABI } from "@/constants/contracts";
import { submitScoreDirectly, canUserSubmit } from "@/utils/directContractInteraction";
...
export default function SubmitButton({
  ...
  // Force the network selection based on which wallet is connected
  useEffect(() => {
    // If ThirdWeb wallet is connected but we're not on polygon network
    if (thirdwebAddress && network !== "polygon") {
      console.log("ThirdWeb wallet is connected - forcing network to polygon");
      setNetwork("polygon");
    }
    // If Wagmi wallet is connected but we're not on base network
    else if (wagmiAccount?.address && network !== "base") {
      console.log("Wagmi wallet is connected - forcing network to base");
      setNetwork("base");
    }
  }, [thirdwebAddress, wagmiAccount?.address, network, setNetwork]);
  ...
  const address = network === "polygon" ? thirdwebAddress : wagmiAccount?.address;
  ...
  const { writeContract, isPending, isSuccess, data: wagmiTxHash } = useWriteContract();
  ...
  const handleSubmit = async () => {
    ...
    if (!walletMatchesNetwork()) {
      toast.error(`Your connected wallet doesn't match the selected ${
    ...
    try {
      ...
      if (!canSubmit.canSubmit) {
        ...
        toast.error(
          `You need to wait ${minutes} minute${minutes > 1 ? "s" : ""} before submitting again`,
          { id: "submit-score" }
        );
        setIsLoading(false);
      ...
      toast.error(`Error submitting score: ${error instanceof Error ? error.message : "Unknown error"}`, {
        id: "submit-score",
      });
      setIsLoading(false);
      setConfirmStep(false);
  ...
  return (
    ...
    >
      {isLoading || isPending ? (
        <>
          <span className="mr-3 text-xl font-bold">SUBMITTING TO {network === "polygon" ? "POLYGON" : "BASE"}...</span>
          <Spinner />
        </>
      ) : confirmStep ? (
        <span className="text-xl font-bold">🔥 CONFIRM SUBMISSION 🔥</span>
      ) : (
        <span className="text-xl font-bold">🏆 SUBMIT TO {network === "polygon" ? "POLYGON" : "BASE"} 🏆</span>
      )}
    </button>
  );
}