...
export default function FarcasterShare({
  ...
  const handleFallbackShare = useCallback(() => {
    ...
    const channelTag = network
      ? `&embeds[]=https://warpcast.com/~/channel/${network}`
      : "";
      ...
      {shareCompleted ? (
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-green-500">
            Your achievement has been shared to the{" "}
            {getNetworkDisplayName(network)} community!
          </p>
        </div>
      ) : (
        <>
