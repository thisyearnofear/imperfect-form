...
const ServerSideSetupSpendLimits: React.FC = () => {
  ...
  // Setup spend limits using server-side API
  const setupSpendLimits = async () => {
    ...
    try {
      ...
      toast.loading("Setting up spend limits via server...", {
        id: "spend-permission",
      });
      ...
      const apiData = {
        spendPermission: {
          ...spendPermission,
          allowance: spendPermission.allowance.toString(), // Convert BigInt to string for JSON
          salt: spendPermission.salt.toString(), // Convert BigInt to string for JSON
        },
        signature,
      };

      // Call our server-side API
      const response = await fetch("/api/spend-permission", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        toast.error(`Failed to set up spend limits: ${data.error}`, {
          id: "spend-permission",
        });
      }
    } catch (error: unknown) {
  ...
  return (
    <div className="p-4 mb-4 bg-yellow-50 border border-yellow-200 rounded-md">
      <h3 className="text-lg font-bold mb-2">
        Enable One-Click Submissions (Server-Side)
      </h3>
      ...
      >
        {isLoading ? (
          ...
          "Set Up Spend Limits (Server-Side)"
        )}
      </button>
    </div>
  );
...
export default ServerSideSetupSpendLimits;
