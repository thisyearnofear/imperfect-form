...
export async function shareViaSdk(content: ShareContent): Promise<ShareResult> {
  try {
    ...
    const imageUrl = content.imageUrl || `${baseUrl}/api/frames/workout/image?reps=${content.reps}&exerciseMode=${content.exerciseMode}&timeSpent=${content.timeSpent}`;
...
export function shareViaFallback(content: ShareContent): ShareResult {
  try {
    ...
    const channelTag = content.network ? `&embeds[]=https://warpcast.com/~/channel/${content.network}` : '';
