...
export function initRemoteLogger(options: {
  ...
  window.addEventListener('error', (event) => {
    sendLog('error', `UNHANDLED ERROR: ${event.message}`, {
...
function sendLog(level: LogLevel, message: string, details?: unknown, context?: string) {
  ...
  if ('connection' in navigator && (navigator as { connection?: { effectiveType?: string } }).connection) {
    payload.clientInfo.networkType = (navigator as { connection: { effectiveType?: string } }).connection.effectiveType;
  }
