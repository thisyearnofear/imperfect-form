...
/**
 * @title FitnessLeaderboardBaseStandardized
 * @dev A leaderboard contract for tracking fitness scores with enhanced reliability
 * Standardized version with consistent struct layout across all networks
 * Specifically optimized for Base Sepolia testnet
 */
contract FitnessLeaderboardBaseStandardized is ReentrancyGuard {
     ...
     */
    function addScore(uint256 _pushups, uint256 _squats) external nonReentrant whenSubmissionsEnabled {
        ...
        // Check cooldown
        uint256 timeRemaining = _getTimeUntilNextSubmission(msg.sender);
    ...
    function getTimeUntilNextSubmission(address _user) external view returns (uint256) {
        return _getTimeUntilNextSubmission(_user);
    }

    /**
     * @dev Internal function to get the time until a user can submit again
     * @param _user Address of the user
     * @return Time in seconds until next submission (0 if can submit now)
     */
    function _getTimeUntilNextSubmission(address _user) internal view returns (uint256) {
